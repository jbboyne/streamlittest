...
df_subset = df_subset.groupby([['Country', 'Date']], as_index = False).rolling(window = 7).mean()
# df_subset = df_subset.join(df_dates)
df_subset = df_subset.groupby([['Country', 'Date']], as_index = False).resample('7D', on = 'Date').last()
